...
public: 
    ...
    friend dynamic_array<T> operator+(const dynamic_array<T>& arr1, dynamic_array<T>& arr2){
        ...
        copy(arr1.begin(), arr1.end(), result.begin()); // result.begin부터 arr1을 일단 복사
        copy(arr2.begin(), arr2.end(), result.begin() + arr1.size()); // result.begin에서 복사한 arr1 바로 뒤부터 arr2 복사
    ...
    string to_string(const string& sep = ", "){
        ...
        for(int i = 1; i<n; i++){
            os << sep << data[i]; // 이때 os << data[i] 이 부분은 << 표준 출력을 수정함 밑에서 그래서 출력방식이 있음.
        }
        return os.str();
    }
};
...
int main(){
    ...
    // 깊은 복사
    auto class2 = class1;
    cout << "1반을 복사하여 2반 생성: " << class2.to_string("그리고") << endl;
    cout << class2[1] << endl;
    // 두 학급을 합쳐서 새로운 큰 학급을 생성
    auto class3 = class1 + class2;
    cout << "1반과 2반을 합쳐 3반 생성: " << class3.to_string("그리고") << endl; 
